...
def is_terminal_state(state):
    if state[0][0] < 0 or state[0][1] < 0 or state[0][0] > (N-1) or state[0][1] > (N-1):
        return True
...
#HYPERPARAMETER ARRAYS FOR HYPERPARAMETER STUDY
number_of_layers_array = [ 1, 2, 3, 4 ]
number_of_unites_in_a_layer_array = [ 10, 25, 50, 75, 100 ]
...
for epoch in range(1, 11):    
    ...
    while episode in range(number_of_episodes):
        ...
        while not is_terminal_state(state) and step < MAX_STEPS and not end:
            ...
            old_y_predictions = y_predictions.numpy().copy()
            old_action = action.copy()
            old_state = state.copy()
            
            state = get_next_state(old_state, old_action)
       ...
       # if episode%sampling_frequency==0 and episode!=0:
